...
class OcleanCustomParamNumber : public number::Number,
                                public Component,
 ...
 public:
  ...
  void setup() override {
    ...
    if (!std::isfinite(v)) v = this->initial_;
    v = this->clamp_to_range_(v);
    this->publish_state(v);
    this->parent_->set_custom_scheme_param(this->kind_, this->index_, clamp_u8_(v),
                                           false);
  }
 ...
 protected:
  void control(float value) override {
    ...
    this->publish_state(value);
    this->parent_->set_custom_scheme_param(this->kind_, this->index_,
                                           clamp_u8_(value), true);
  }

  static uint8_t clamp_u8_(float v) {
    return (uint8_t) std::max(0L, std::min((long) UINT8_MAX, lroundf(v)));
  }
