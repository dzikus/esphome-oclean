...
find "${config_dir}/.esphome/build" -path "*/components/oclean/*" -name "*.obj" -delete 2>/dev/null || true
...
sources=("$(dirname "${BASH_SOURCE[0]}")/../components/oclean"/*.cpp)
built="$(grep -oE "Building CXX object [^ ]*components/oclean/[a-z_]+\.cpp" "${lines}" | sort -u | wc -l)"
...
if grep -E "components/oclean/.*(warning|error):" "${lines}"; then
