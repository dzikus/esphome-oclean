...
entries = [e for e in json.load(open(db_path)) if '/components/oclean/' in e['file']]
if not entries:
    sys.exit('compile database holds no oclean translation unit')
...
clang-tidy -p "${work}" --header-filter='.*/components/oclean/.*' "${sources[@]}" > "${report}" 2>&1 || true
...
if grep -E '/components/oclean/[^:]+:[0-9]+:[0-9]+: (warning|error):' "${report}"; then
